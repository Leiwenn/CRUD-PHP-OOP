    ...
    <section class="pt-4 offset-md-2 col-md-8 col-12 offset-md-2">
        <div>
            ...
            <form class="form_comment bg-white shadow p-3 m-3 rounded" action="index.php?action=comment&postId=<?= $postId ?>" method="post">
                <p>Pseudo: 
                    <?php
                    ...
                    <?php
                        if(isset($_SESSION['pseudo'])){
                    ?>
                    <label class="mt-2" for="title">Titre du message</label>
                    <input class="form-control" type="text" name="title" id="title" placeholder="titre" required>
                    <label class="mt-2" for="comment">Votre commentaire</label>
                    <textarea rows="5" id="comment" name="comment" placeholder="commentaire" class="form-control" required></textarea>
