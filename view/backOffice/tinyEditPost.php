        ...
        <form action="index.php?action=update_post&id=<?= $data['id'] ?>&title=<?= $data['title'] ?>&content=<?= $data['content'] ?>&file_name=<?= $data['file_name'] ?>&file_description=<?= $data['file_description'] ?>" method="post">
            <div class="form-group border rounded bg-white mt-4 p-2">
                <div class="row p-2">
                    ...
                    <label class="m-2 p-2 col-12 col-md-6 shadow rounded text-info" for="img"><i class="far fa-file-image" aria-hidden="true"></i> IMAGE
                    ...
                    <label class="m-2 p-2 col-12 col-md-6 shadow rounded text-info" for="alt_text">Texte alternatif à l'image
