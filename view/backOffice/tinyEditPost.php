    <section class="editor p-4">
        ...
        ?>
        <form action="index.php?action=update_post&id=<?= $data['id'] ?>&title=<?= $data['title'] ?>&content=<?= $data['content'] ?>&file_name=<?= $data['file_name'] ?>&file_description=<?= $data['file_description'] ?>" method="post" enctype="multipart/form-data">
            <div class="form-group mt-4 d-flex justify-content-around align-items-baseline flex-wrap">
                ...
                <label class="p-2 col-12 col-md-4 shadow rounded text-info bg-light" for="img"><i class="far fa-file-image" aria-hidden="true"></i> IMAGE
                    <input type="file" accept=".png, .jpg, .jpeg" class="form-control-file pt-3" id="img" name="file_name">
            ...
            <div class="form-group border rounded bg-white mt-4 p-2">
                ...
                <label class="m-2 text-info" for="tiny_post"><i class="fas fa-paragraph" aria-hidden="true"></i> TEXTE</label>
